...
def output_sample(data, num_sample):
  ...
  for image_data in to_output:
    ...
    display(PIL.Image.fromarray(np.uint8(data)))
...
def main(argv=None):  # pylint: disable=unused-argument
  ...
  def get_correct_data(data, sess, labels):
  ...
  with tf.Session() as sess:
    ...
    correct_data, correct_labels = get_correct_data(test_data, sess, test_labels)
    print('Correct data size:', len(correct_data))
    ...
    attack_data = new_data + EPSILON * numpy.sign(var_grad_val[0])
    output_sample(attack_data, 20)

    test_error = error_rate(eval_in_batches(attack_data, sess), new_labels)
    print('Error rate: %.1f%%' % test_error)
